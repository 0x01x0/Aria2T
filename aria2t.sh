...
check_pid() {
	PID=$(ps -ef | grep "aria2c" | grep -v grep | grep -v "aria2t.sh" | grep -v "service" | awk '{print $2}')
}
...
Download_aria2_conf() {
    PROFILE_URL1="https://cdn.jsdelivr.net/gh/P3TERX/aria2.conf@master"
    PROFILE_LIST="
aria2.conf
clean.sh
core
...
Update_bt_tracker() {
    ...
    check_pid
    [[ -z $PID ]] && {
        bash <(wget -qO- https://cdn.jsdelivr.net/gh/P3TERX/aria2.conf@master/tracker.sh) ${aria2_conf}
    } || {
        bash <(wget -qO- https://cdn.jsdelivr.net/gh/P3TERX/aria2.conf@master/tracker.sh) ${aria2_conf} RPC
    }
    echo -en "\n\n点击任意键以继续"
    read -n 1 line
}
...
Update_Shell() {
    sh_new_ver=$(wget -qO- -t1 -T3 "https://raw.githubusercontent.com/0x01x0/Aria2T/main/aria2t.sh" | grep 'sh_ver="' | awk -F "=" '{print $NF}' | sed 's/\"//g' | head -1) && sh_new_type="github"
    [[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
    if [ -f "$PREFIX/etc/tconfig/aria2.sht.bak2" ]; then
	    rm -f $PREFIX/etc/tconfig/aria2.sht.bak2
    fi
    if [ -f "$PREFIX/etc/tconfig/aria2t.sh.bak" ]; then
	    mv $PREFIX/etc/tconfig/aria2t.sh.bak $PREFIX/etc/tconfig/aria2.sht.bak2
    fi
    if [[ -d $PREFIX/etc/tconfig ]]; then
	    echo "检测到 Termux Tools! 启用 Termux Tools 更新方案!"
	    mv $PREFIX/etc/tconfig/aria2t.sh $PREFIX/etc/tconfig/aria2t.sh.bak
	    wget -P $PREFIX/etc/tconfig https://raw.githubusercontent.com/0x01x0/Aria2T/main/aria2t.sh && chmod +x $PREFIX/etc/tconfig/aria2t.sh
	    return 0
    else
	    wget -N "https://raw.githubusercontent.com/0x01x0/Aria2T/main/aria2t.sh" && chmod +x aria2t.sh
    fi
